...
export default function Drawing() {
    ...
    if (activeUserCanvas.activeImageData) {
      ...
      if (canvas) {
        ...
        if (ctx) {
          ...
          image.src = URL.createObjectURL(new Blob([decodedImage], { type: 'image/png' }));
        }
      }
    } else {
      updateCanvas();
    }
  }, [activeUserCanvas.activeImageData, updateCanvas]);
  ...
  }, [drawingStore.lineColor, drawingStore.lineOpacity, drawingStore.lineWidth]);
  ...
  return (
    <div className="bg-slate-700 h-screen w-full flex flex-col items-center justify-center relative">
      ...
      <div className="flex flex-col gap-4 bg-white absolute top-5 left-4 shadow-2xl p-4 rounded-xl min-w-[250px]">
        ...
        <Menu canvasRef={canvasRef} ctxRef={ctxRef} />
      </div>
